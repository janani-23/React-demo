...
export type RemoveColorCallbak = (color:IColor) =>void ;

export interface IColorListProps{
    colors:IColor[];
    onRemoveColor:RemoveColorCallbak;
}
export class ColorList extends React.Component<IColorListProps, {}> {
   ...
   private _onRenderListCell=(color:IColor, index:number | undefined):JSX.Element=>{
      return (
          ...
          {color.title}<br/>
          <DefaultButton text="delete" data={color.id} onClick={ () =>this._onButtonClick(color)}    />
